...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body::before {
    ...
    inset: 0;
    background-image:
      linear-gradient(rgba(0,48,135,0.025) 1px, transparent 1px),
      linear-gradient(90deg, rgba(0,48,135,0.025) 1px, transparent 1px);
  ...
  header::after {
    ...
    height: 4px;
    background: linear-gradient(90deg, var(--gold), #ffd166, var(--gold));
  }
  .header-inner {
  ...
  .dropzone {
    ...
    transition: all 0.25s; background: var(--surface2); margin-bottom: 14px;
  }
  ...
  .skill-tag { font-size: 0.7rem; padding: 3px 9px; border-radius: 5px; font-weight: 600; font-family: 'Open Sans', sans-serif; }
  ...
  .contact-btn {
    ...
    color: var(--muted); border-radius: 7px; padding: 9px;
  ...
  <div class="layout">
    ...
    <div class="card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="jobTitle" placeholder="Ej: Desarrollador Backend Senior" />
      </div>
      <div class="form-group">
...
<script>
// ╔══════════════════════════════════════════════════════════════╗
// ║  REEMPLAZA CON LA URL DE TU APPS SCRIPT DE ESCÁNER CV       ║
// ╚══════════════════════════════════════════════════════════════╝
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/TU_URL_ESCANERCV_AQUI/exec';
...
function handleFiles(files) {
  ...
  if (allowed.length < files.length) showToast('⚠️ Solo se permiten archivos .txt, .pdf y .docx');
  allowed.forEach(f => {
...
async function guardarEnSheets(r, puesto) {
  ...
  try {
    ...
    note.textContent = '✅ Guardado en Google Sheets correctamente';
    note.className = 'ok';
  } catch (e) {
    note.textContent = '⚠️ No se pudo guardar en Sheets (revisa la URL del script)';
    note.className = 'err';
  }
}
...
function renderCard(r, idx) {
  ...
  card.innerHTML = `
    ...
    <button class="contact-btn ${r.status === 'eligible' ? 'eligible-contact' : ''}"
      onclick="copyContact('${r.contact_email||''}','${r.contact_phone||''}','${r.name}')">
      ${contactInfo}
    </button>`;
