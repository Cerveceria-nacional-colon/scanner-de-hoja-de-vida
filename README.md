...
<!-- Librerías para leer PDF y DOCX -->
...
async function readFile(file){
  ...
  try {
    ...
    if(ext === 'doc'){
      return '[Archivo .doc antiguo: conviértelo a .docx o PDF para mejor lectura]';
    }
...
// NO SE CAMBIARON LOS CAMPOS QUE VAN AL APPS SCRIPT
...
async function analyzeCV(filename, cvText, jobTitle, jobDesc){
  ...
  if(!cleanCV || cleanCV.length < 40){
    ...
    return {
      ...
      score: 40,
      matched_skills: [],
      missing_skills: extractJobRequirements(cleanJob).slice(0, 8),
      summary: 'No se pudo extraer suficiente texto de esta hoja de vida. Revisa si el archivo es una imagen escaneada o si está protegido.',
      contact_email: '',
      contact_phone: ''
    };
  }
  ...
  jobRequirements.forEach(req => {
    if(candidateSkills.includes(req) || cleanCV.includes(req)){
      matched.push(req);
    } else {
      missing.push(req);
    }
  ...
  const extraRelevantSkills = candidateSkills.filter(skill => !matched.includes(skill));
  const allMatched = [...matched, ...extraRelevantSkills.slice(0, 6)];

  const educationScore = calculateEducationScore(cleanCV, cleanJob);
  const experienceScore = calculateExperienceScore(cleanCV, cleanJob);
  const skillsScore = calculateSkillsScore(matched, jobRequirements, candidateSkills);
  const contactScore = email || phone ? 5 : 0;

  let score = educationScore + experienceScore + skillsScore + contactScore;
  ...
  const summary = buildSummary(name, jobTitle, score, allMatched, missing, cleanCV);
...
function detectCandidateSkills(text){
  const dictionary = {
    'python': ['python'],
    'sql': ['sql', 'base de datos', 'bases de datos', 'database'],
    'excel': ['excel', 'microsoft excel'],
    'word': ['word', 'microsoft word'],
    'powerpoint': ['powerpoint', 'power point'],
    'office': ['office', 'microsoft office', 'paquete microsoft office'],
    'linux': ['linux', 'ndg linux'],
    'aws': ['aws', 'amazon web services', 'cloud', 'nube'],
    'redes': ['redes', 'networking', 'cisco', 'ccna'],
    'ciberseguridad': ['ciberseguridad', 'cybersecurity', 'seguridad informatica'],
    'inventario': ['inventario', 'stock', 'almacen'],
    'logistica': ['logistica', 'despacho', 'carga', 'operaciones logisticas'],
    'reportes': ['reporte', 'reportes', 'informes'],
...
function extractJobRequirements(jobText){
  ...
  possible.forEach(req => {
    if(jobText.includes(normalizeText(req))){
      if(req === 'base de datos') requirements.push('sql');
      else if(req === 'cisco') requirements.push('redes');
      else if(req === 'almacen') requirements.push('inventario');
      else if(req === 'despacho') requirements.push('logistica');
...
function calculateEducationScore(cv, job){
  ...
  if(cv.includes('universidad') || cv.includes('licenciatura')) score += 15;
...
function calculateExperienceScore(cv, job){
  ...
  if(cv.includes('experiencia')) score += 10;
  ...
  if(cv.includes('almacen')) score += 8;
  if(cv.includes('remoto')) score += 5;
  if(/\b2024\b|\b2025\b|\b2026\b/.test(cv)) score += 5;
...
function calculateSkillsScore(matched, requirements, candidateSkills){
  ...
  score += Math.min(candidateSkills.length * 3, 15);

  return Math.min(score, 40);
}

function buildSummary(name, jobTitle, score, matched, missing, cv){
  const cargo = jobTitle || 'la vacante indicada';

  let perfil = `${name} presenta una compatibilidad de ${score}% para ${cargo}.`;

  if(matched.length){
    perfil += ` Destaca por conocimientos o experiencia en ${matched.slice(0, 5).join(', ')}.`;
  }

  if(missing.length){
    perfil += ` Se recomienda validar en entrevista aspectos como ${missing.slice(0, 3).join(', ')}.`;
  } else {
    perfil += ` El perfil cubre los principales requisitos definidos para el cargo.`;
  }
